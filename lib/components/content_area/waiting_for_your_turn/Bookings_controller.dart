...
//               btnName = 'buy'.tr;
//               showBtn = true;
//               final double? price = item['session']?['price']?.toDouble();
//               paymentPrice = price;
//               break;
...
//     // consoleLog('pay');
//     PayMobManager().getPaymentKey(paymentPrice, 'EGP').then((
//       String paymentKey,
//     ) {
//       // launchUrl(
//       //   Uri.parse(
...
//   Future<void> changeAvailableBtn(String status) async {
//     switch (status) {
//       case 'Confirmed':
//         _pay();
//         break;
...
class BookingsController extends GetxController {
  ...
  Future<void> _pay(int doctorId) async {
    PayMobManager().payment(loukMyPatientId, doctorId);
  ...
  Future<void> changeAvailableBtn(
    String status,
    int doctorId,
    int secionId,
  ) async {
